...
class SpecialistAgent {
    ...
    async process(userQuestion, routing, session, qwenSessionId) {
        try {
            ...
            if (this.isNeracaRequest(userQuestion, routing)) {
                console.log(`📊 Detected Neraca request, using custom tool`);
            ...
            if (this.isLabaRugiRequest(userQuestion, routing)) {
                console.log(`📊 Detected Laba Rugi request, using custom tool`);
            ...
            if (this.isSaldoKasRequest(userQuestion, routing)) {
                console.log(`📊 Detected Saldo Kas request, using custom tool`);
                return await this.processSaldoKasRequest(userQuestion, session);
            }

            // Check if user is asking for Buku Besar (General Ledger)
            if (this.isBukuBesarRequest(userQuestion, routing)) {
                console.log(`📊 Detected Buku Besar request, using custom tool`);
                return await this.processBukuBesarRequest(userQuestion, session);
            }

            // Check if user is asking for data available via OpenAPI/API
    ...
    isNeracaRequest(question, routing) {
        ...
        return neracaKeywords.some((keyword) =>
    ...
    isLabaRugiRequest(question, routing) {
        ...
        const lowerQuestion = question.toLowerCase();
        return labaRugiKeywords.some((keyword) =>
            lowerQuestion.includes(keyword),
        );
    }
    ...
    isSaldoKasRequest(question, routing) {
        ...
        const lowerQuestion = question.toLowerCase();
        return saldoKasKeywords.some((keyword) =>
            lowerQuestion.includes(keyword),
        );
    }
    ...
    isBukuBesarRequest(question, routing) {
        ...
        const lowerQuestion = question.toLowerCase();
        return bukuBesarKeywords.some((keyword) =>
            lowerQuestion.includes(keyword),
        );
    }
    ...
    async processNeracaRequest(question, session) {
        try {
            ...
            console.log(
                `📊 Executing Neraca tool for ${session.tenantSchema}: ${period.start_date} to ${period.end_date}`,
            );
    ...
    async processLabaRugiRequest(question, session) {
        try {
            ...
            console.log(
                `📊 Executing Laba Rugi tool for ${session.tenantSchema}: ${period.start_date} to ${period.end_date}`,
            );
    ...
    async processSaldoKasRequest(question, session) {
        try {
            ...
            console.log(
                `📊 Executing Saldo Kas tool for ${session.tenantSchema}: as of ${as_of_date}`,
            );
    ...
    async processBukuBesarRequest(question, session) {
        try {
            ...
            console.log(
                `📊 Executing Buku Besar tool for ${session.tenantSchema}: ${start_date} to ${end_date}`,
            );
