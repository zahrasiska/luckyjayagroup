-- Active: 1767458714744@@127.0.0.1@5432@luckyjayagroup
-- PostgreSQL version of the original MariaDB query
-- Schema: {schema} (tenant data) with prive (supporting data for all tenants)
         ...
         (SELECT ja.tahun,
          ...
          LEFT JOIN
              (SELECT j.rek,
               ...
               FROM {schema}.j
               INNER JOIN {schema}.t ON t.id = j.idtrans
               AND j.rek != 340.00
               GROUP BY j.rek,
                        EXTRACT(YEAR
                                FROM t.tanggal),
               ...
               FROM
                   (SELECT EXTRACT(YEAR
                           ...
                           SUM(CASE
                               ...
                               END) AS rl,
                           SUM(CASE
                                   WHEN j.rek = 340.00 THEN COALESCE(j.debit, 0)-COALESCE(j.kredit, 0)
                                   ELSE 0
                               END) AS d
                    FROM {schema}.j
