...
class LabaRugiTool {
    constructor() {
        ...
        // Path to rugi_laba_periodic.sql
        this.sqlFilePath = path.join(
            __dirname,
            "..",
            "sql",
            "rugi_laba_periodic.sql",
        );
    }
    ...
    async execute(params) {
        ...
        try {
            ...
            sqlTemplate = sqlTemplate.replace(
                /inner join t on/g,
                `inner join ${schema}.t on`,
            );

            // Replace parameter placeholders
            const sql = sqlTemplate
                .replace(/:tgl1/g, `'${start_date}'`)
                .replace(/:tgl2/g, `'${end_date}'`)
                .replace(/:iddevisi/g, iddevisi);
